...
/// Managed state pointing at the AI debug log file. Every AI completion
/// response is appended here so model-side issues can be diagnosed offline.
pub struct AiDebugLogState {
    pub log_path: std::path::PathBuf,
}

/// Append one AI response to the debug log. Best-effort: any I/O error is
/// swallowed so logging never breaks an AI call.
fn append_ai_debug_log(log_path: &Path, model: &str, content: &str) {
    use std::io::Write;
...
#[tauri::command]
pub async fn ai_complete(debug_log: State<'_, AiDebugLogState>, request: AiCompletionRequest) -> Result<String, String> {
    let model = request.config.model.clone();
    let result = dbx_core::ai::complete(&request).await;
    if let Ok(content) = &result {
        append_ai_debug_log(&debug_log.log_path, &model, content);
    }
    result
}

#[tauri::command]
pub async fn ai_stream(app: AppHandle, debug_log: State<'_, AiDebugLogState>, session_id: String, request: AiCompletionRequest) -> Result<(), String> {
    let cancelled = dbx_core::ai::register_stream(&session_id).await;
    let model = request.config.model.clone();
    // The chunk callback is Fn (not FnMut), so accumulate through a Mutex.
    ...
    if result.is_ok() {
        let content = collected.lock().map(|buf| buf.clone()).unwrap_or_default();
        append_ai_debug_log(&debug_log.log_path, &model, &content);
    }
    result
}
