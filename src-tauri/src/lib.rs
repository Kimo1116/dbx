...
fn should_show_main_window_after_setup() -> bool {
    // Window is shown by the frontend via the "frontend-ready" event once the UI
    // has painted its first frame, avoiding the white flash on startup.
    false
}
...
mod tests {
    ...
    #[test]
    fn defers_main_window_until_frontend_is_ready() {
...
pub fn run() {
    ...
    builder
        ...
        .setup(move |app| {
            ...
            app.manage(commands::ai::AiDebugLogState { log_path: data_dir.join("logs").join("ai-debug.log") });
            app.manage(commands::sql_review::ReviewSettingsState { settings_path: data_dir.join("review_settings.json") });
            ...
            if should_show_main_window_after_setup() {
                ...
                // Show the window once the frontend has painted its first frame.
                let handle = app.handle().clone();
                app.listen("frontend-ready", move |_event| {
                    show_main_window(&handle);
                });
                // Fallback: guarantee the window appears even if the event never fires.
                let fallback_handle = app.handle().clone();
                tauri::async_runtime::spawn(async move {
                    tokio::time::sleep(std::time::Duration::from_secs(6)).await;
                    show_main_window(&fallback_handle);
                });
            }
            #[cfg(any(windows, target_os = "linux"))]
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::sql_review::sql_review_run,
            commands::sql_review::sql_review_list_rules,
            commands::sql_review::sql_review_load_settings,
            commands::sql_review::sql_review_save_settings,
