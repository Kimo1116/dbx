...
const ReviewPanel = defineAsyncComponent(() => import("@/components/review/ReviewPanel.vue"));
const AiReviewPanel = defineAsyncComponent(() => import("@/components/review/AiReviewPanel.vue"));
...
const showReviewPanel = ref(safeLocalStorageGet("dbx-review-panel-open") === "true");
const showAiReviewPanel = ref(safeLocalStorageGet("dbx-ai-review-panel-open") === "true");
...
const rightSidebarPanelStorageKeys: Partial<Record<RightSidebarPanelId, string>> = {
  ...
  review: "dbx-review-panel-open",
  aiReview: "dbx-ai-review-panel-open",
};
...
const STARTUP_STEPS: { label: string; pct: number }[] = [
  { label: "正在初始化界面", pct: 8 },
  { label: "正在加载 AI 配置", pct: 30 },
  { label: "正在加载编辑器设置", pct: 52 },
  { label: "正在恢复会话标签", pct: 72 },
  { label: "正在加载连接配置", pct: 90 },
  { label: "加载完成", pct: 100 },
];
const startupStepIndex = ref(0);
const appReady = ref(false);
const startupProgress = computed(() => STARTUP_STEPS[startupStepIndex.value]?.pct ?? 100);
const startupStepLabel = computed(() => STARTUP_STEPS[startupStepIndex.value]?.label ?? "");
const showSplash = computed(() => isDesktop && !appReady.value);
const { sidebarWidth, aiPanelWidth, historyWidth, sqlLibraryWidth, sqlFilePanelWidth, reviewWidth, aiReviewWidth, startSidebarResize, startAiPanelResize, startHistoryResize, startSqlLibraryResize, startSqlFilePanelResize, startReviewResize, startAiReviewResize } = usePanelResize();
...
function handleRunRuleReview() {
  openRightSidebarPanel("review");
}

function handleRunAiReview() {
  openRightSidebarPanel("aiReview");
}
...
function handleKeydown(e: KeyboardEvent) {
  ...
  if ((e.ctrlKey || e.metaKey) && e.shiftKey && (e.key === "R" || e.key === "r")) {
    e.preventDefault();
    e.stopPropagation();
    toggleRightSidebarPanel("review");
    return;
  }
...
async function initApp() {
  ...
  startupStepIndex.value = 1;
  await settingsStore.initAiConfigs();
  try {
    startupStepIndex.value = 2;
...
onMounted(async () => {
  ...
  if (isDesktop) {
    // Notify the Rust side after the first frame is actually painted so the
    // hidden main window can be shown without a white flash. Double rAF
    // guarantees the initial paint has happened.
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
  ...
  void initApp().finally(async () => {
    startupStepIndex.value = STARTUP_STEPS.length - 1;
...
<template>
  ...
  <div v-show="!setupRequired && (!needsAuth || authenticated)" class="fixed inset-0 h-screen w-screen overflow-hidden">
    <TooltipProvider :delay-duration="300">
      <div class="h-screen w-screen max-w-full min-w-[760px] min-h-[600px] flex flex-col bg-background text-foreground overflow-hidden" :class="{ 'dbx-desktop-window-frame': drawDesktopWindowFrame }" :style="appUiFontFamilyStyle">
        <AppToolbar
          ...
          :show-review-panel="showReviewPanel"
          ...
          @run-rule-review="handleRunRuleReview"
          @run-ai-review="handleRunAiReview"
        ...
        <div :class="isClassicLayout ? 'app-layout-classic flex-1 flex min-h-0' : 'app-panel-gutter flex-1 flex min-h-0 gap-1 p-1'">
          ...
          <div v-show="showReviewPanel" :class="isClassicLayout ? 'h-full shrink-0 relative z-30 isolate bg-background' : 'h-full shrink-0 relative z-30 isolate rounded-md border border-border/80 bg-background'" :style="{ width: reviewWidth + 'px' }">
            <div class="panel-resize-handle panel-resize-handle--left" @mousedown="startReviewResize" />
            <div class="h-full min-h-0 overflow-hidden">
              <ReviewPanel
                :sql="activeTab?.sql ?? ''"
                :dialect="activeConnection?.db_type ?? 'mysql'"
                :connection-id="activeTab?.connectionId"
                :database="activeTab?.database"
                @close="closeRightSidebarPanel('review')"
              />
            </div>
          </div>

          <div v-show="showAiReviewPanel" :class="isClassicLayout ? 'h-full shrink-0 relative z-30 isolate bg-background' : 'h-full shrink-0 relative z-30 isolate rounded-md border border-border/80 bg-background'" :style="{ width: aiReviewWidth + 'px' }">
            <div class="panel-resize-handle panel-resize-handle--left" @mousedown="startAiReviewResize" />
            <div class="h-full min-h-0 overflow-hidden">
              <AiReviewPanel
                :sql="activeTab?.sql ?? ''"
                :dialect="activeConnection?.db_type ?? 'mysql'"
                :connection-id="activeTab?.connectionId"
                :database="activeTab?.database"
                @close="closeRightSidebarPanel('aiReview')"
              />
            </div>
          </div>
    ...
    <Transition name="dbx-splash-fade">
      <div v-if="showSplash" class="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-background">
        <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500 to-purple-700 shadow-lg shadow-purple-900/40">
        ...
        <div class="mt-5 text-xl font-semibold tracking-[0.3em] text-foreground">DBX</div>
        <div class="mt-1.5 text-[11px] text-muted-foreground">Database Workspace</div>
        <div class="mt-9 h-1 w-60 overflow-hidden rounded-full bg-muted">
          <div class="h-full rounded-full bg-primary transition-all duration-500 ease-out" :style="{ width: startupProgress + '%' }" />
        </div>
        <div class="mt-3 text-xs text-muted-foreground">{{ startupStepLabel }}</div>
      </div>
    </Transition>
  </div>
</template>
...
.dbx-splash-fade-leave-active {
  transition: opacity 0.45s ease;
}
.dbx-splash-fade-leave-to {
  opacity: 0;
}
</style>
