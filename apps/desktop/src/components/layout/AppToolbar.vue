...
import { DatabaseZap, FilePlus2, Moon, Sun, SunMoon, History, Bot, ArrowLeftRight, FileCode, BookMarked, GitCompareArrows, TableProperties, Settings, Package, FileDown, FolderTree, ShieldCheck, Sparkles } from "@lucide/vue";
...
const collapsibleRightItemDefs = computed(() => {
  ...
  if (toolbarItems.value.review) {
    items.push({
      key: "review",
      label: "规则审查",
      icon: ShieldCheck,
      action: () => emit("run-rule-review"),
      disabled: false,
    });
  }
  if (toolbarItems.value.aiReview) {
    items.push({
      key: "aiReview",
      label: "AI 审查",
      icon: Sparkles,
      action: () => emit("run-ai-review"),
      disabled: false,
    });
  }
...
<template>
  <div ref="toolbarEl" class="app-toolbar h-10 flex items-center gap-1 px-2 border-b bg-muted/30 shrink-0 overflow-hidden" :style="toolbarStyle" data-tauri-drag-region @dblclick="onToolbarDblClick">
    ...
    <div ref="rightWrapper" class="flex min-w-0 items-center gap-1 overflow-hidden">
      ...
      <Tooltip v-if="toolbarItems.review">
        <TooltipTrigger as-child>
          <Button v-show="isRightItemVisible('review')" variant="ghost" size="icon" class="h-8 w-8 shrink-0" :class="{ 'bg-accent': showReviewPanel }" @click="emit('run-rule-review')">
            <ShieldCheck class="h-4 w-4" />
          </Button>
        </TooltipTrigger>
        <TooltipContent>规则审查</TooltipContent>
      </Tooltip>

      <Tooltip v-if="toolbarItems.aiReview">
        <TooltipTrigger as-child>
          <Button v-show="isRightItemVisible('aiReview')" variant="ghost" size="icon" class="h-8 w-8 shrink-0" :class="{ 'bg-accent': showAiReviewPanel }" @click="emit('run-ai-review')">
            <Sparkles class="h-4 w-4" />
          </Button>
        </TooltipTrigger>
        <TooltipContent>AI 审查</TooltipContent>
      </Tooltip>
