...
import { DEFAULT_AI_REVIEW_PROMPT } from "@/lib/review/sqlReview";
...
type SettingsCategory = "editor" | "formatter" | "appearance" | "navigation" | "data" | "backups" | "tunnels" | "shortcuts" | "snippets" | "sync" | "ai" | "review" | "mcp" | "security" | "about";
...
// ---------- Review Settings ----------
const editReviewPrompt = ref("");
const reviewPromptSaving = ref(false);
const reviewPromptDirty = ref(false);
...
const REVIEW_SEVERITY_LABELS: Record<string, { text: string; cls: string }> = {
  error: { text: "错误", cls: "text-red-500" },
  warning: { text: "警告", cls: "text-amber-500" },
  info: { text: "提示", cls: "text-blue-500" },
  style: { text: "风格", cls: "text-muted-foreground" },
};

const reviewRulesGrouped = computed(() => {
  const groups: Array<{ category: string; label: string; rules: typeof reviewSettingsStore.rules }> = [];
  const byCategory = new Map<string, typeof reviewSettingsStore.rules>();
  for (const rule of reviewSettingsStore.rules) {
    const list = byCategory.get(rule.category) ?? [];
    list.push(rule);
    byCategory.set(rule.category, list);
  }
  for (const [category, rules] of byCategory) {
    groups.push({ category, label: REVIEW_CATEGORY_LABELS[category] ?? category, rules });
  }
  return groups;
});

function loadReviewPromptIntoEditor() {
  editReviewPrompt.value = reviewSettingsStore.effectivePromptTemplate;
  reviewPromptDirty.value = false;
}

async function saveReviewPrompt() {
  reviewPromptSaving.value = true;
  try {
    await reviewSettingsStore.setSystemPrompt(editReviewPrompt.value);
    reviewPromptDirty.value = false;
    toast("AI 审查提示词已保存");
  } catch (e) {
    toast(`保存失败: ${String(e)}`);
  } finally {
    reviewPromptSaving.value = false;
  }
}
...
async function resetReviewPrompt() {
  reviewPromptSaving.value = true;
  try {
    await reviewSettingsStore.resetSystemPrompt();
    editReviewPrompt.value = DEFAULT_AI_REVIEW_PROMPT;
    reviewPromptDirty.value = false;
    toast("已恢复默认提示词");
  } catch (e) {
    toast(`保存失败: ${String(e)}`);
...
async function toggleReviewRule(ruleId: string, enabled: boolean) {
  try {
    await reviewSettingsStore.setRuleEnabled(ruleId, enabled);
  } catch (e) {
    toast(`保存失败: ${String(e)}`);
  }
}

async function resetReviewRules() {
  try {
    await reviewSettingsStore.resetRuleOverrides();
    toast("已恢复默认规则配置");
  } catch (e) {
    toast(`保存失败: ${String(e)}`);
  }
...
const REVIEW_PROMPT_PLACEHOLDERS = [
  { token: "{{dialect}}", desc: "替换为当前 SQL 方言" },
  { token: "{{database}}", desc: "替换为当前数据库名" },
  { token: "{{sql}}", desc: "替换为待审查的 SQL 语句" },
  { token: "{{rule_summary}}", desc: "替换为规则引擎已发现的问题清单" },
];

const reviewPromptTextarea = ref<HTMLTextAreaElement | null>(null);

function insertReviewPlaceholder(token: string) {
  const el = reviewPromptTextarea.value;
  const value = editReviewPrompt.value;
  if (!el) {
    editReviewPrompt.value = value + token;
  } else {
    const start = el.selectionStart ?? value.length;
    const end = el.selectionEnd ?? start;
    editReviewPrompt.value = value.slice(0, start) + token + value.slice(end);
    void nextTick(() => {
      el.focus();
      ...
      <div class="settings-layout flex min-h-0 flex-1 flex-col gap-3 overflow-hidden lg:flex-row">
        ...
        <div class="min-w-0 flex-1 overflow-hidden px-1 flex flex-col">
          <div class="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-1 pr-2">
                ...
                <div class="grid grid-cols-3 gap-2 mt-2">
                  <div
                    v-for="item in [
                      ...
                      { key: 'ai', label: 'AI' },
                      { key: 'review', label: '规则审查' },
                      { key: 'aiReview', label: 'AI 审查' },
            ...
            <section v-else-if="activeSettingsTab === 'review'" class="flex flex-col gap-5 py-2">
              <!-- Rule engine rules -->
              <div class="space-y-3">
                <div class="flex items-center justify-between">
                  <div>
                    <h3 class="text-sm font-medium">规则审查</h3>
                    <p class="text-xs text-muted-foreground">选择启用的内置审查规则，关闭后审查时将跳过对应检查</p>
                  </div>
                  <Button variant="outline" size="sm" class="h-7 text-xs" :disabled="!reviewSettingsStore.isLoaded" @click="resetReviewRules">恢复默认</Button>
                </div>

                <div v-if="!reviewSettingsStore.isLoaded" class="flex items-center gap-2 py-8 text-xs text-muted-foreground">
                  <Loader2 class="h-4 w-4 animate-spin" />
                  加载规则列表...
                </div>

                <div v-else class="space-y-4">
                  <div v-for="group in reviewRulesGrouped" :key="group.category" class="rounded-md border bg-muted/20">
                    <div class="border-b border-border/60 px-3 py-2 text-xs font-medium text-muted-foreground">{{ group.label }}</div>
                    <div class="divide-y divide-border/40">
                      <div v-for="rule in group.rules" :key="rule.id" class="flex items-center gap-3 px-3 py-2.5">
                        <div class="min-w-0 flex-1">
                          <div class="flex items-center gap-2">
                            <span class="font-mono text-[10px] text-muted-foreground">{{ rule.id }}</span>
                            <span class="truncate text-xs">{{ rule.name }}</span>
                            <span class="shrink-0 text-[10px]" :class="REVIEW_SEVERITY_LABELS[rule.severity]?.cls ?? 'text-muted-foreground'">{{ REVIEW_SEVERITY_LABELS[rule.severity]?.text ?? rule.severity }}</span>
                          </div>
                        </div>
                        <Switch :model-value="reviewSettingsStore.isRuleEnabled(rule)" @update:model-value="(v: boolean) => toggleReviewRule(rule.id, v)" />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="space-y-3">
                <div class="flex items-center justify-between">
                  <div>
                    <h3 class="text-sm font-medium">AI 审查提示词</h3>
                    <p class="text-xs text-muted-foreground">用于 AI 深度审查的系统提示词模板，可直接编辑优化</p>
                  </div>
                  <div class="flex items-center gap-2">
                    <Button variant="outline" size="sm" class="h-7 text-xs" :disabled="!reviewSettingsStore.isLoaded || reviewPromptSaving" @click="resetReviewPrompt">恢复默认</Button>
                    <Button size="sm" class="h-7 text-xs" :disabled="!reviewPromptDirty || reviewPromptSaving" @click="saveReviewPrompt">
                      {{ reviewPromptSaving ? "保存中..." : "保存" }}
                    </Button>
                  </div>
                </div>

                <div class="space-y-1.5 rounded-md border border-border/60 bg-muted/20 px-3 py-2">
                  <p class="text-[11px] text-muted-foreground">
                    审查时，模板中的占位符会自动替换为实际内容。点击占位符可插入到光标处。
                    <span v-if="reviewSettingsStore.hasCustomPrompt" class="ml-1 text-primary">当前使用自定义提示词。</span>
                  </p>
                  <div class="flex flex-wrap gap-1.5">
                    <button
                      v-for="ph in REVIEW_PROMPT_PLACEHOLDERS"
                      :key="ph.token"
                      type="button"
                      class="group inline-flex items-center gap-1 rounded border border-border/60 bg-background px-1.5 py-0.5 font-mono text-[11px] text-foreground/80 transition-colors hover:border-primary/50 hover:text-primary"
                      :title="ph.desc"
                      @click="insertReviewPlaceholder(ph.token)"
                    >
                      {{ ph.token }}
                      <span class="font-sans text-[10px] text-muted-foreground group-hover:text-primary/70">{{ ph.desc }}</span>
                    </button>
                  </div>
                </div>

                <textarea
                  ref="reviewPromptTextarea"
                  v-model="editReviewPrompt"
                  class="w-full rounded-md border bg-background px-3 py-2 text-xs font-mono leading-relaxed resize-y min-h-[280px]"
                  rows="14"
                  spellcheck="false"
                  @input="reviewPromptDirty = true"
                ></textarea>
              </div>
            </section>
