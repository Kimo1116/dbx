...
async function send() {
  ...
  if (!props.connection || !props.tab) {
    toast(t("editor.selectConnection"));
    return;
  }
  if (!props.tab.database) {
    toast(t("editor.selectDatabaseRequired"));
    return;
  }
  if (!settings.isConfigured) {
    toast(t("ai.noConfig"));
      ...
      <div ref="promptPanelRef" class="relative rounded-[6px] border bg-background">
        ...
        <div class="px-2 pb-2 pt-1">
          ...
          <div class="flex min-w-0 flex-nowrap items-center gap-1.5 overflow-hidden">
            ...
            <button v-else class="h-7 w-7 shrink-0 rounded-full bg-foreground text-background flex items-center justify-center disabled:opacity-30" :disabled="!prompt.trim() && !selectedMentions.length && !selectedSqlFileMentions.length" @click="send">
