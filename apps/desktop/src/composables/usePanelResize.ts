...
export function usePanelResize() {
  ...
  const reviewWidth = ref(Number(safeLocalStorageGet("dbx-review-width")) || 420);
  const aiReviewWidth = ref(Number(safeLocalStorageGet("dbx-ai-review-width")) || 480);
  ...
  const startReviewResize = startPanelResize(reviewWidth, "dbx-review-width", "left");
  const startAiReviewResize = startPanelResize(aiReviewWidth, "dbx-ai-review-width", "left");
